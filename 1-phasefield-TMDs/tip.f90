...
PROGRAM pffd
  ...
  DOUBLE PRECISION :: c1,c2,cf,x,y,r,dphi,D1,D2,ts,F,psi,tphi,xi_eq,xi_inf,k,n,g,tmp,sump,&
          &pi = 4.D0*DATAN(1.D0),dx,dy,t,dt
  DOUBLE PRECISION, DIMENSION(:,:), ALLOCATABLE :: phi,phi0,dphidt,xi,dxidt,phitemp,grad_phi_x,&
          &grad_phi_y,angle,ax,ay,kappa2,kappa_prime,&
          &kappa,dxdy,dydx,grad_kappa2_x,grad_kappa2_y,lap_phi,lap_xi,&
          &term1,term2,term3
  DOUBLE PRECISION, DIMENSION(:) :: sump_done(150)
  ...
  ALLOCATE(phi(nx,ny),phi0(nx,ny),dphidt(nx,ny),xi(nx,ny),dxidt(nx,ny),grad_phi_x(nx,ny),phitemp(nx,ny),&
...
!!--------------------------------------------------------------------------
  ...
  xi_inf=xi_eq*4D0
  ts=xi_inf/F;
  sump_done=nx*ny*(-1d0)+2d0/512*nx*ny* &
     & (/ (i,i=1,150) /)
...
!!! Ceate initial condions
  DO i = 1,nx
    DO j = 1,ny
      x = DBLE(i)*dx
      y = DBLE(j)*dx
      ! A: (c1-r/2D0*DSQRT(3),c2+r/2D0), B:(c1+r/2D0*SQRT(3),c2+r/2D0), C:(c1,x2-r)
!      IF ((y .GE. (c1-r/2D0*DSQRT(3D0))) .and. (y .LE. (c1+r/2D0*DSQRT(3D0))) .and. (x .GE. (c2-r)) .and. (x .LE. (c2+r/2D0))) THEN
!        IF (y .LE. c1) THEN
!          IF ((y-(c1-r/2D0*DSQRT(3D0))) .GE. (c2+r/2D0-x)/DSQRT(3D0) ) THEN
!            phi0(i,j) =  1.D0
!          ELSE
!            phi0(i,j) =  -1.D0
!          ENDIF
!        ELSE
!          IF ( c1+r/2D0*DSQRT(3D0)-y .GE. (c2+r/2D0-x)/DSQRT(3D0) ) THEN
!            phi0(i,j) =  1.D0
!          ELSE
!            phi0(i,j) =  -1.D0
!          ENDIF
  ...
  !  term2=dsin(pi*phi)+psi*(xi-xi_eq)*(1d0+dcos(pi*phi))
    DO i = 1, nx
      DO j = 1, ny
        x = DBLE(i)*dx
        IF(x .LT. cf) THEN
          term2(i,j)=dsin(pi*phi(i,j))+psi*(xi(i,j)-3D0*xi_eq)*(1d0+dcos(pi*phi(i,j)))
        ELSE
          term2(i,j)=dsin(pi*phi(i,j))+psi*(xi(i,j)-xi_eq)*(1d0+dcos(pi*phi(i,j)))
        ENDIF
      ENDDO
    ENDDO
    ...
    DO i = 1, nx
      DO j = 1, ny
        x = DBLE(i)*dx
        IF(x .LT. cf) THEN
          xi(i,j)=xi(i,j)+(D1*lap_xi(i,j)+(F-xi(i,j)/ts)*(1d0-phi(i,j))/2d0)*DT-0.5d0*dphidt(i,j);
!        ELSEIF (x .GT. c1+r) THEN
!          xi(i,j)=xi(i,j)+-0.5d0*dphidt(i,j);
        ELSE
          xi(i,j)=xi(i,j)+(D2*lap_xi(i,j))*DT-0.5d0*dphidt(i,j);
        ENDIF
      ENDDO
    ENDDO
...
!!--------------------------------------------------------------------------
    ...
    DO i = 1,nx
      DO j = 1,ny
        IF (phitemp(i,j) > 1.D0) then
          phitemp(i,j) = 1.D0
        END IF
      ENDDO
    ENDDO
    sump=sum(phitemp)
    if(iter>=5000000)exit
